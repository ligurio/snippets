/*
 * Example of client to use with https://github.com/Microsoft/WindowsProtocolTestSuites
 *
 */
...
const void *read_touch_event_single_payload(const void *buffer, touch_event_single *event) {
  buffer = read_uint32(buffer, &event->touch_event_times);
  /* buffer = read_uint16(buffer, &event->positions); */
}

const void *read_touch_event_multiple_payload(const void *buffer, touch_event_multiple *event) {
  buffer = read_uint32(buffer, &event->touch_position_count);
  /* buffer = read_uint16(buffer, &event->positions); */
}

const void *read_display_update_resolution_payload(const void *buffer, display_update_resolution *update) {
...
const void *read_rdp_connection_payload(const void *buffer, connection_parameters *conn) {
...
void print_request_msg(request_msg *msg) {
  ...
  if (msg->display_update_resolution) {
    ...
    printf("Display update resolution: %d, %d x %d, %d\n", update->operation, update->width,
					update->height, update->orientation);
  }
  printf("\n");
}
...
int decode_request_msg(const void *buffer, request_msg *msg) {
  ...
  switch (msg->command_id) {
    ...
    case DISPLAY_UPDATE_RESOLUTION:
         ...
         // read_display_update_resolution_payload(buffer, &msg->display_update_resolution);
         break;
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    case START_RDP_CONNECTION:
        ...
        rc = system("/usr/bin/xfreerdp /u:Administrator /p:Password01 /v:10.16.170.5:3389 &");
    ...
    // system("xfreerdp /t:RDPClient /rfx /u:Administrator /p:Password01 /v:10.16.170.5:3389");

    /*
    freerdp: https://github.com/FreeRDP/FreeRDP/blob/master/client/common/client.c
    freerdp: https://github.com/FreeRDP/FreeRDP/blob/master/client/Sample/tf_freerdp.c
    */
  }
